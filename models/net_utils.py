...
class Feature_Pool(nn.Module):
    ...
    def forward(self, x):
        b, c, _, _ = x.size()
        y = self.MLP(self.gap_pool(x).permute(0, 2, 3, 1)).permute(0, 3, 1, 2).view(b, c)
        return y


class Channel_Attention(nn.Module):
    def __init__(self, dim, ratio=32):
    ...
    def forward(self, x):
        x = self.dwconv(x)
        max_out = self.MLP(self.gmp_pool(x).permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        return max_out
...
class EAEF(nn.Module):
    def __init__(self, dim):
        ...
        self.ese = Channel_Attention(dim)
    ...
    def forward(self, RGB, T):
        ...
        t_y = t_y.view(b, 1, c)
        logit_scale = c  # * self.logit_scale
        logits_per = logit_scale * rgb_y @ t_y
        cross_gate = torch.diagonal(self.sigmoid(logits_per)).reshape(b, c, 1, 1)
        ...
        fuse_gate = self.cse(x_cat)
        rgb_gate, t_gate = fuse_gate[:, 0:c, :], fuse_gate[:, c:c * 2, :]
        ##########################################################################
        New_fusion = (RGB+T) * add_gate
        con_gate = self.ese(New_fusion)
        ##########################################################################
        New_RGB = RGB * con_gate + RGB * rgb_gate
        New_T = T * con_gate + T * t_gate
        ##########################################################################
